...
export let options = {
  vus: 800, // Número de usuarios virtuales
...
export default function () {
  ...
  group('User Flow: Nextcloud', function () {
    ...
    // Acceso a Collabora (vista de archivo)
    ...
    // Acceso a Actividad
    const activityRes = http.get(`${BASE_URL}/login?redirect_url=/apps/activity/all`, {
      jar: jar,
    });
    check(activityRes, {
      '5. Activity page loaded': (r) => r.status === 200,
    });
    sleep(2);

    // Acceso a Calendario
    const calendarRes = http.get(`${BASE_URL}/login?redirect_url=/apps/calendar/dayGridMonth/now`, {
      jar: jar,
    });
    check(calendarRes, {
      '6. Calendar page loaded': (r) => r.status === 200,
    });
    sleep(2);

    // Acceso a Contactos
    const contactsRes = http.get(`${BASE_URL}/login?redirect_url=/apps/contacts/Todos%20los%20contactos/QWRtaW5pc3RyYWRvcn56LXNlcnZlci1nZW5lcmF0ZWQtLXN5c3RlbQ==`, {
      jar: jar,
    });
    check(contactsRes, {
      '7. Contacts page loaded': (r) => r.status === 200,
    });
    sleep(2);

    // Acceso a Notas
    const notesRes = http.get(`${BASE_URL}/login?redirect_url=/apps/notes/welcome`, {
      jar: jar,
    });
    check(notesRes, {
      '8. Notes page loaded': (r) => r.status === 200,
    });
    sleep(2);

    // Acceso a Formularios
    const formsRes = http.get(`${BASE_URL}/login?redirect_url=/apps/forms/`, {
      jar: jar,
    });
    check(formsRes, {
      '9. Forms page loaded': (r) => r.status === 200,
    });
    sleep(2);

    // Descargar un archivo específico (Antiguo punto 5, ahora 10)
    ...
    check(downloadRes, {
      '10. File downloaded': (r) => r.status === 200,
    });
